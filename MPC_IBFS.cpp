...
//find minflow by reducing the maximal amount of flow (with maxflow) from a feasible flow
void find_minflow_ibfs_alt(ListDigraph& g, ListDigraph::ArcMap<int>& demands, ListDigraph::ArcMap<int>& flow, ListDigraph::Node s, ListDigraph::Node t, int max_paths)
{
  ...
  for(ListDigraph::ArcIt a(g); a != INVALID; ++a){
    if(g.source(a) == s){
      g_ibfs->addNode(labels_ibfs[g.target(a)], feasible_flow[a] - demands[a], 0);
    }
    else if(g.target(a) == t){
      g_ibfs->addNode(labels_ibfs[g.source(a)], feasible_flow[a] - demands[a], 0);
    }
    else{
      g_ibfs->addEdge(labels_ibfs[g.source(a)], labels_ibfs[g.target(a)], feasible_flow[a]-demands[a], 0, arc_labels[a]);    
    }
  }
